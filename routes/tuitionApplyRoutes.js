...
// ONE-TIME MIGRATION ROUTE — backfill missing phone numbers from RegTeacher
// Also sets isSpam / isBest / isExpress from the Phone collection.
// SAFE: only touches records where phone is empty/null. Uses $set on 4 fields only.
// Remove this route after running once.
router.post('/backfill-phones', async (req, res) => {
    try {
        // Load active phone list for spam/best/express lookup
        const phoneList = await Phone.find({ isActive: true }).lean();

        // Find all apply records with missing phone
        ...
        const results = {
            total: missingPhoneRecords.length,
            updated: 0,
            skippedNoPremiumCode: 0,
            skippedNoTeacherMatch: 0,
            skippedNoPhoneOnTeacher: 0,
            details: []
        };
        ...
        for (const record of missingPhoneRecords) {
            ...
            if (!premiumCode || premiumCode.trim() === '') {
                results.skippedNoPremiumCode++;
                results.details.push({ tuitionCode, name, reason: 'no premiumCode' });
                continue;
            }
            ...
            if (!teacher) {
                results.skippedNoTeacherMatch++;
                results.details.push({ tuitionCode, name, reason: `premiumCode "${premiumCode}" not in RegTeacher` });
                continue;
            }
            ...
            if (!normalizedPhone) {
                results.skippedNoPhoneOnTeacher++;
                results.details.push({ tuitionCode, name, reason: `teacher "${teacher.name}" has no phone` });
                continue;
            }

            // Determine isSpam / isBest / isExpress from Phone collection
            const normalizedForCompare = normalizePhone(normalizedPhone);
            let isSpam = false;
            let isBest = false;
            let isExpress = false;

            for (const entry of phoneList) {
                const dbNormalized = normalizePhone(entry.phone);
                if (dbNormalized === normalizedForCompare) {
                    if (entry.isSpam) isSpam = true;
                    else if (entry.isExpress) isExpress = true;
                    else if (entry.isBest) isBest = true;
                    break;
                }
            }
            ...
            // Update ONLY phone, isSpam, isBest, isExpress
            await TuitionApply.findByIdAndUpdate(
                _id,
                { $set: { phone: normalizedPhone, isSpam, isBest, isExpress } },
                { new: false }
            );

            results.updated++;
            results.details.push({ tuitionCode, name, phone: normalizedPhone, tag, status: 'updated' });
        }
        ...
        res.json({
            ...
            summary: {
                ...
                skippedNoPremiumCode: results.skippedNoPremiumCode,
                skippedNoTeacherMatch: results.skippedNoTeacherMatch,
                skippedNoPhoneOnTeacher: results.skippedNoPhoneOnTeacher,
            },
